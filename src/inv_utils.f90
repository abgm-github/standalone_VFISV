...
CONTAINS
  ...
  PURE SUBROUTINE GET_LAMBDA(DELTACHI2,LAMBDA_OLD,LAMBDA_NEW)
    ...
    ELSE ! Things got worse
       ...
       IF (LAMBDA_OLD.gt.0.01d0) LAMBDA_NEW=2.0D0*LAMBDA_NEW
    ENDIF
  ...
  ! in the routine that calculates the model improvement (GET_DMODEL).
    ...
    DO I=1,NUMFREE_PARAM
       DO J=1,I
          HELP=0.0D0
          DO K=1,4
             HELP=HELP+(WEIGHTS(K)**2.0D0)*SUM(DSYN(FREELOC(I),:,K)*DSYN(FREELOC(J),:,K))
          ENDDO
          HESS(I,J)=(2.0D0/NUMFREE_DEG)*HELP
          HESS(J,I)=(2.0D0/NUMFREE_DEG)*HELP
       ENDDO
    ENDDO
  ...
  SUBROUTINE GET_DMODEL(MODEL, REGUL_FLAG, DIVC, HESS, LAMBDA, DMODEL, CONV_FLAG)
    ...
    DMODEL=0.0D0 ! In case parameter is not free or SVD fails.
  ...
  ! Marquardt (1+ lambda) factor applied to diagonal of Hessian matrix
    ...
    DO I = 1, NUMFREE_PARAM
       DO J = 1, NUMFREE_PARAM
          HESS1(j,i) = HESS(j,i)
       ENDDO
       HESS1(i,i)=HESS1(i,i)*(1.0D0+LAMBDA)
    ENDDO
   ...
   ,NUMFREE_PARAM,VT,NUMFREE_PARAM,WORK,15*NUMFREE_PARAM,INFO)
    ...
    ELSE
       ...
       WMAX=MAXVAL(W)
       DO I=1,NUMFREE_PARAM
          IF (W(I).LT.SVDTOL*WMAX) W(I)=0.0D0
       ENDDO
       ...
       DO I=1,NUMFREE_PARAM
          MX=PLUSMODEL(I)
          IF ((MX.eq.(MX+1.0D0)).or.(abs(MX).gt.1.0D10)) then
             CONV_FLAG=2
          ELSE
             DMODEL(FREELOC(I))=MX
          ENDIF
