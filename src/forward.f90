MODULE FORWARD
  ...
  !
  ! By ABGM, January 2021: Added SYNTHESIS2 routine that calculated a nonmagnetic component. It is
  ! assumed that the thermodynamic parameters for the nonmagnetic component and
  ! for the magnetic one are the same. 
...
CONTAINS
  ...
  !!
  !! SUBROUTINE SYNTHESIS2 
  !!
  SUBROUTINE SYNTHESIS2(MODEL,SCAT,DERIVATIVE,SYN,DSYN,FILTERS,INTEG_FILTERS)
    USE FILT_PARAM, ONLY: NBINS
    USE LINE_PARAM, ONLY: NUMW
    USE CONS_PARAM, ONLY: DP
   
    IMPLICIT NONE
    REAL(DP),              DIMENSION(10)          :: MODEL
    REAL(DP),              DIMENSION(10)          :: MODEL_ORIG
    REAL(DP), INTENT(INOUT),  DIMENSION(NBINS,4)  :: SCAT
    REAL(DP), INTENT(IN),  DIMENSION(NUMW, NBINS) :: FILTERS
    REAL(DP), INTENT(IN),  DIMENSION(NBINS)       :: INTEG_FILTERS
    LOGICAL,  INTENT(IN)                          :: DERIVATIVE
    REAL(DP), INTENT(OUT),  DIMENSION(NBINS,4)    :: SYN
    REAL(DP), INTENT(OUT),  DIMENSION(10,NBINS,4) :: DSYN
    REAL(DP), DIMENSION(10,NBINS,4)               :: DSYN_M
    REAL(DP), DIMENSION(10,NBINS,4)               :: DSYN_NM
    REAL(DP), DIMENSION(NBINS,4)    :: SYN_M
    REAL(DP), DIMENSION(NBINS,4)    :: SYN_NM
    INTEGER                                       :: I
    !------------------------------------------------------
  
    !SAVE THE GUESS MODEL
    MODEL_ORIG(:) = MODEL(:)



    !! 1) SYNTHESIS OF THE NON-MAGNETIZED PROFILE
    SCAT(:,:) = 0.0D0
    MODEL(2) = 0.0D0
    MODEL(3) = 0.0D0
    MODEL(6) = 0.0D0
    MODEL(10) = 1.0D0
    CALL SYNTHESIS(MODEL,SCAT,DERIVATIVE,SYN_NM,DSYN_NM,FILTERS,INTEG_FILTERS)




    !! 2) SYNTHESIS OF THE MAGNETIC PROFILE 
    MODEL(:) = MODEL_ORIG(:)
    MODEL(10) = 1.0D0
    ...
    IF (DERIVATIVE.EQV..TRUE.) THEN
      !! Q, U, V e Imag
      DSYN(1:9,:,:) = MODEL(10)*DSYN_M(1:9,:,:) 
      !! I no mag
      DO I=1,9
        IF ((I.EQ.2).OR.(I.EQ.3).OR.(I.EQ.6)) CYCLE
        DSYN(I,:,1) = DSYN(I,:,1) + (1.-MODEL(10))*DSYN_NM(I,:,1)
      ENDDO
      DSYN(10,:,:) = SYN_M - SYN_NM
    ENDIF

    !! As I need scat after the inversion, I save the syn_nm in scat
    SCAT(:,:) = SYN_NM(:,:)

  END SUBROUTINE SYNTHESIS2
  !!
  !! SOUBROUTINE ABSMAT
  !!
