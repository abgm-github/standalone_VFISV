SUBROUTINE INVERT (OBS_LONG,SCAT_LONG,GUESS,RES,ERR, FILTERS_LONG, CONVERGENCE_FLAG, WEIGHTS, POL_LOW,POL_UPP)
!SUBROUTINE INVERT (OBS_LONG,SCAT_LONG,GUESS,RES,ERR, FILTERS_LONG, CONVERGENCE_FLAG, WEIGHTS, POLARIZATION)
  ...
  REAL(DP), INTENT(INOUT),  DIMENSION(NBINS*4)          :: SCAT_LONG
  !REAL(DP), INTENT(IN),  DIMENSION(NBINS*4)          :: SCAT_LONG
  !abgm
  !abgm la linea siguiente la tengo que descomentar si quiero modificar la forma de la
  !functio del SCAT_LONG
  !REAL(DP), INTENT(INOUT),  DIMENSION(NBINS*4)          :: SCAT_LONG
  !abgm
  ...
  INTEGER                               :: CONV_FLAG, CONVERGENCE_FLAG, NRESET, CHANGEVAR_FLAG &
           , REGUL_FLAG, RESET_FLAG, DONE, ITSTART, ABANDON, N_ABANDON
  !---------------------------------------------------------------------
  REAL(DP),     DIMENSION(NBINS,4)      :: SYN, LASTGOODSYN, BESTSYN
  REAL(DP),     DIMENSION(10,NBINS,4)   :: DSYN, LASTGOODDSYN
  !---------------------------------------------------------------------
  REAL(DP)                              :: POL,POL_LOW,POL_UPP  !!abgm
...
! Predict S0/S1 from Doppler width. Maintain S0+S1
        ...
        RESET_FLAG=0
        ABANDON=0
        if ((nreset.eq.1).and.((i-itstart).ge.N_ABANDON).and.((lastgoodchi2-bestminchi2).gt.0).and. &
                  (bestmodel(6).lt.BNOISE)) ABANDON=1 ! Restart is not getting better.
