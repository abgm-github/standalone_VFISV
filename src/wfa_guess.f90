SUBROUTINE WFA_GUESS(OBS,LAMBDA_0,VELOCITY, AZIMUTH, ETA0, DOPPLERW, S0, S1)
...
! The following is slightly better
  ...
  IF (MAXVAL(ABS(OBS(:,4))) .GT. 2.0D0*NOISE(1)) &
       VELOCITY = SUM(ABS(OBS(:,4))*TUNEPOS)/SUM(ABS(OBS(:,4)))
  VELOCITY=1.0D-3*(2.998D10/LAMBDA_0)*VELOCITY    ! cm/s
